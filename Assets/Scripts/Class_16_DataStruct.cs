    ...
    public class Class_16_DataStruct : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            foreach (var item in playerDefens)
            {
                ...
                LogSystem.LogWithColor($"{props.Count}", "#77f");
                // Capcity : 指清單內的容量，系統自動分配，預設為 4 並且以倍數成長 (C# 各版本不同)
                LogSystem.LogWithColor($"{props.Capacity}", "#77f");
            }
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                LogSystem.LogWithColor($"容量 : {number.Capacity}", "#f39");
            }
            #endregion
            ...
            SortedSet<int> lv = new SortedSet<int> { 7, 3, 78, 123, 5, 80 };
            // 交集與差集
            ...
            // 2.SortedDictionart 是使用樹狀結構方式儲存，比較占記憶體空間
            ...
            // 如果資料不需要頻繁的增減建議使用 SortedList 否之建議使用 SortedDictionary
