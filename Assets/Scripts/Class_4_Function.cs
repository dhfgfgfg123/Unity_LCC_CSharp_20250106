...
public class Class_4_Function : MonoBehaviour
{
    private void Awake()
    {
        ...
        Debug.Log($"<color=#f22>{BMI()}</color>");
        Debug.Log($"<color=#f22>{BMI(62, 1.63f)}</color>");

        Move();
        Move(50.5f);
    }
    // 框選要整理程式 > Ctrl + K + S > #region 快速完成程式碼片段
    #region 方法基本語法與參數
    // 方法
    // 包含一系列程式區塊
    // 方法語法 :
    ...
    private void SpawnEnemy(string enemy = "洞穴哥布林")
    {
    ...
    //{
    //    Debug.Log($"<color=#f66>發射物件 : {fire} , 速度 : </color>");
    //}
    ...
    /// <returns></returns>
    private float BMI(float weight = 60, float height = 1.68f)
    {
